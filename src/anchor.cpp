...
AnchorGenerator::AnchorGenerator(const string &class_name,
                                 ...
                                 const Array3f &offset) : _class_name(class_name)
{
    ...
    _anchors = MatrixXf(xyz_range[0] * xyz_range[1] * xyz_range[2] * 2, 7);
    ...
    for (int z = 0; z < xyz_range[2]; ++z)
    {
        for (int y = 0; y < xyz_range[1]; ++y)
        {
            for (int x = 0; x < xyz_range[0]; ++x)
            {
                ...
                // cout << "x = " << x << " y = " << y << " z = " << z << " cur_id = " << cur_id << endl;
...
MatrixXf AnchorGenerator::get_anchors()
{
    return _anchors;
}
