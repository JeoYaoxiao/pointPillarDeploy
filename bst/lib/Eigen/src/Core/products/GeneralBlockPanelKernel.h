                   ...
                   typename enable_if<unpacket_traits<Packet>::size==16>::type* = 0)
{
  ...
  res.first  = predux_half_dowto4(CplxPacket(a.first));
  res.second = predux_half_dowto4(CplxPacket(a.second));
  return res;
}
...
DoublePacket<typename unpacket_traits<Packet>::half>
predux_half_dowto4(const DoublePacket<Packet> &a,
                   typename enable_if<unpacket_traits<Packet>::size==32>::type* = 0)
{
  // yes, that's pretty hackish :(
  DoublePacket<typename unpacket_traits<Packet>::half> res;
  typedef std::complex<typename unpacket_traits<Packet>::type> Cplx;
...
template<typename Scalar, typename RealPacket>
void loadQuadToDoublePacket(const Scalar* b, DoublePacket<RealPacket>& dest,
                            typename enable_if<unpacket_traits<RealPacket>::size==32>::type* = 0)
{
  // yes, that's pretty hackish too :(
  typedef typename NumTraits<Scalar>::Real RealScalar;
  RealScalar r[8] = {numext::real(b[0]), numext::real(b[0]),numext::real(b[0]), numext::real(b[0]), numext::real(b[1]), numext::real(b[1]),numext::real(b[1]), numext::real(b[1])};
  RealScalar i[8] = {numext::imag(b[0]), numext::imag(b[0]),numext::imag(b[0]), numext::imag(b[0]), numext::imag(b[1]), numext::imag(b[1]),numext::imag(b[1]), numext::imag(b[1])};
  dest.first  = ploadquad<RealPacket>(r);
  dest.second = ploadquad<RealPacket>(i);
}
